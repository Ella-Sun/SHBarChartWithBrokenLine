...
- (void)processYMaxValue {
    NSArray *yAxisValues = _yLabels ? _yLabels : _yValues[0];
...
- (void)updateBar
{
    ...
    for (int i = 0; i< _yValues.count; i++) {
        ...
        for (int j = 0; j < childArray.count; j++) {
            ...
            NSNumber * valueString = childArray[j];
            ...
            barWidth = _xLabelWidth * perWidth;
            barXPosition = (j+perXP) *  _xLabelWidth + _chartMarginLeft + i*_xLabelWidth * perLab;// (i+1) * barWidth ;
            
            batHeight = self.showLevelLine ? chartCavanHeight/2.0:chartCavanHeight;
            barYPosition = self.frame.size.height - chartCavanHeight - kXLabelHeight - _chartMarginTop;
            CGRect barFrame = CGRectMake(barXPosition, barYPosition, barWidth, batHeight);
            
            [self createEachBar:valueString withBarFrame:barFrame barIndex:j+i*childArray.count];
...
//创建每一个bar
- (void)createEachBar:(NSNumber *)valueString withBarFrame:(CGRect)barFrame barIndex:(NSInteger)index{
    ...
    if ([[currentNumber substringToIndex:1] isEqualToString:@"-"]) {
//        bar.isNegative = YES;
    }
...
- (UIColor *)barColorAtIndex:(NSUInteger)index
{
    NSUInteger childElement;
    NSArray *childAry = self.yValues[0];
    if (_yValues.count > 1) {
        childElement = childAry.count * _yValues.count;
    } else {
        childElement = childAry.count;
    }
    if (self.strokeColors.count == childElement) {
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSLog(@"%ld",subview.tag);
    if ([subview isKindOfClass:[PNBar class]] && [self.delegate respondsToSelector:@selector(userClickedOnBarAtIndex:atPoint:)]) {
        [self.delegate userClickedOnBarAtIndex:subview.tag atPoint:touchPoint];
    }
}
